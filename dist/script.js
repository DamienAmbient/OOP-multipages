...
class VideoPlayer {
  constructor(triggers, overlay) {
    ...
    this.onPalyerStateChange = this.onPalyerStateChange.bind(this);
  }
  createPlayer(url) {
    this.player = new YT.Player("frame", {
      ...
      videoId: `${url}`,
      events: {
        onStateChange: this.onPalyerStateChange
      }
    });
    this.overlay.style.display = "flex";
  }
  onPalyerStateChange(state) {
    try {
...
class MainSlider extends _slider__WEBPACK_IMPORTED_MODULE_0__["default"] {
  ...
  bindTriggers() {
    ...
    document.querySelectorAll(".nextodule").forEach(item => {
      item.addEventListener("click", e => {
        e.stopPropagation();
        e.preventDefault();
        this.plusSlides(1);
